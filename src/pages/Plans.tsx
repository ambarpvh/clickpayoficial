            ...
            return (
              <div key={plan.id} className={`glass-card rounded-xl p-6 border-2 ${color} relative ${isCurrent ? "glow-primary" : ""} ${isSoldOut ? "opacity-80" : ""} hover:scale-105 transition-transform duration-300`}>
                ...
                <div className="space-y-3 text-sm text-muted-foreground mb-6">
                  <p className="flex items-center gap-2"><Check className="h-4 w-4 text-primary" /> <p className="flex items-center gap-2"><Check className="h-4 w-4 text-primary" /> Ganho por anúncio: <span className="text-primary font-semibold">{formatBRL(plan.click_value)}</span></p> className="text-primary font-semibold">{formatBRL(plan.click_value)}</span></p>
